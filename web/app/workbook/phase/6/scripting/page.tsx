...
import { ScriptingEditor, ScriptingData } from '@/components/workbook/Phase6/ScriptingEditor'
import { useAuth } from '@/hooks/useAuth'
import { useAutoSave } from '@/hooks/useAutoSave'
import { supabase } from '@/lib/supabase'

const DEFAULT_DATA: ScriptingData = {
  script: '',
}
...
export default function ScriptingPage() {
  ...
  const { user } = useAuth()
  const [data, setData] = useState<ScriptingData>(DEFAULT_DATA)
    ...
    const loadData = async () => {
      try {
        ...
        if (error && error.code !== 'PGRST116') {
          ...
          setData(existing.data as ScriptingData)
        }
      } catch (err) {
