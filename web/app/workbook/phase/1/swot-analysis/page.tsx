...
import { SWOTEditor, SWOTData } from '@/components/workbook/Phase1/SWOTEditor'
...
export default function SWOTAnalysisPage() {
  ...
  const { user } = useAuth()
  const [data, setData] = useState<SWOTData>(DEFAULT_DATA)
    ...
    const loadData = async () => {
      try {
        ...
        if (error && error.code !== 'PGRST116') {
          ...
          setData(existing.data as SWOTData)
        }
      } catch (err) {
  ...
  return (
    <WorksheetLayout
      ...
      previousLabel="Wheel of Life"
      nextLabel="Habits Audit"
    >
      <SWOTEditor data={data} onChange={setData} />
    </WorksheetLayout>
  )
}
