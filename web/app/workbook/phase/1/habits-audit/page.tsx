...
import { HabitsAuditEditor, HabitsAuditData } from '@/components/workbook/Phase1/HabitsAuditEditor'
...
export default function HabitsAuditPage() {
  const router = useRouter()
  const { user } = useAuth()
  const [data, setData] = useState<HabitsAuditData>(DEFAULT_DATA)
    ...
    const loadData = async () => {
      try {
        ...
        if (error && error.code !== 'PGRST116') {
          ...
          setData(existing.data as HabitsAuditData)
        }
      } catch (err) {
  ...
  return (
    <WorksheetLayout
      ...
      onPrevious={() => router.push('/workbook/phase/1/swot-analysis')}
      previousLabel="Back to SWOT Analysis"
    >
      <HabitsAuditEditor data={data} onChange={setData} />
    </WorksheetLayout>
  )
}
