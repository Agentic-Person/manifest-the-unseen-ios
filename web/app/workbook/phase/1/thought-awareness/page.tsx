...
import { ThoughtAwarenessEditor, ThoughtAwarenessData } from '@/components/workbook/Phase1/ThoughtAwarenessEditor'
...
export default function ThoughtAwarenessPage() {
    ...
    const loadData = async () => {
      try {
        const { data: existing, error } = await supabase
          ...
          .select('data')
          .eq('user_id', user.id)
          .eq('worksheet_id', 'thought-awareness')
          .eq('phase_number', 1)
          .single()
        ...
        if (error && error.code !== 'PGRST116') {
          ...
          setData(existing.data as ThoughtAwarenessData)
        }
      } catch (err) {
  ...
  const { status, lastSaved } = useAutoSave({
    ...
    onSave: async (currentData) => {
      ...
      const { error } = await supabase
        ...
        .upsert({
          user_id: user.id,
          worksheet_id: 'thought-awareness',
          phase_number: 1,
          data: currentData,
  ...
  return (
    <WorksheetLayout
      ...
      onPrevious={() => router.push('/workbook/phase/1/abilities-rating')}
      previousLabel="Back to Abilities Rating"
    >
      <ThoughtAwarenessEditor data={data} onChange={setData} />
    </WorksheetLayout>
  )
}
