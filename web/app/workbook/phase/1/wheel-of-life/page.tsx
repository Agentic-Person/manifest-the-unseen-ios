...
import { WheelOfLifeEditor, WheelOfLifeData } from '@/components/workbook/Phase1/WheelOfLifeEditor'
...
export default function WheelOfLifePage() {
  ...
  const { user } = useAuth()
  const [data, setData] = useState<WheelOfLifeData>(DEFAULT_DATA)
    ...
    const loadData = async () => {
      try {
        ...
        if (error && error.code !== 'PGRST116') {
          ...
          setData(existing.data as WheelOfLifeData)
        }
      } catch (err) {
  ...
  return (
    <WorksheetLayout
      ...
      previousLabel="Back to Phase 1"
    >
      <WheelOfLifeEditor data={data} onChange={setData} />
    </WorksheetLayout>
  )
}
