...
import { GratitudeLettersEditor, GratitudeLettersData } from '@/components/workbook/Phase7/GratitudeLettersEditor'
import { useAuth } from '@/hooks/useAuth'
import { useAutoSave } from '@/hooks/useAutoSave'
import { supabase } from '@/lib/supabase'

const DEFAULT_DATA: GratitudeLettersData = {
  letters: [],
}

export default function GratitudeLettersPage() {
  const router = useRouter()
  const { user } = useAuth()
  const [data, setData] = useState<GratitudeLettersData>(DEFAULT_DATA)
    ...
    const loadData = async () => {
      try {
        const { data: existing, error } = await supabase
          ...
          .select('data')
          .eq('user_id', user.id)
          .eq('worksheet_id', 'gratitude-letters')
          .eq('phase_number', 7)
          .single()
        ...
        if (error && error.code !== 'PGRST116') {
          ...
          setData(existing.data as GratitudeLettersData)
        }
      } catch (err) {
