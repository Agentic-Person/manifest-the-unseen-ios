...
import { GratitudeMeditationEditor, GratitudeMeditationData } from '@/components/workbook/Phase7/GratitudeMeditationEditor'
import { useAuth } from '@/hooks/useAuth'
import { useAutoSave } from '@/hooks/useAutoSave'
import { supabase } from '@/lib/supabase'

const DEFAULT_DATA: GratitudeMeditationData = {
  sessions: [],
}

export default function GratitudeMeditationPage() {
  const router = useRouter()
  const { user } = useAuth()
  const [data, setData] = useState<GratitudeMeditationData>(DEFAULT_DATA)
    ...
    const loadData = async () => {
      try {
        const { data: existing, error } = await supabase
          ...
          .select('data')
          .eq('user_id', user.id)
          .eq('worksheet_id', 'gratitude-meditation')
          .eq('phase_number', 7)
          .single()
        ...
        if (error && error.code !== 'PGRST116') {
          ...
          setData(existing.data as GratitudeMeditationData)
        }
      } catch (err) {
