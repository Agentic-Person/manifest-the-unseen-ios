...
import { GraduationEditor, GraduationData } from '@/components/workbook/Phase10/GraduationEditor'
...
export default function GraduationPage() {
  ...
  const { user } = useAuth()
  const [data, setData] = useState<GraduationData>(DEFAULT_DATA)
    ...
    const loadData = async () => {
      try {
        ...
        if (error && error.code !== 'PGRST116') {
          ...
          setData(existing.data as GraduationData)
        }
      } catch (err) {
  ...
  return (
    <WorksheetLayout
      ...
      onPrevious={() => router.push('/workbook/phase/10/future-letter')}
    >
      <GraduationEditor data={data} onChange={setData} />
    </WorksheetLayout>
  )
}
