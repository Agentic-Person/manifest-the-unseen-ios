  ...
  return (
    <div className={`space-y-8 ${className}`}>
      <div className="bg-purple-50 border border-purple-200 rounded-lg p-4">
        <h4 className="font-semibold text-purple-900 mb-1">WOOP Method</h4>
        <p className="text-sm text-purple-800">Wish, Outcome, Obstacle, Plan - A proven mental strategy for achieving goals.</p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {STEPS.map((step, index) => (
          <div key={step.key} className="bg-white rounded-xl border-2 border-purple-200 p-6">
            <div className="flex items-center gap-2 mb-4">
              <div className="w-8 h-8 bg-purple-600 text-white rounded-full flex items-center justify-center font-bold">
                {index + 1}
              </div>
              <h3 className="text-lg font-semibold text-purple-900">{step.label}</h3>
            </div>
