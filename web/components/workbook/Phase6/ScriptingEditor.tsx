  ...
  return (
    <div className={`space-y-8 ${className}`}>
      <div className="bg-elevated border border-[rgba(196,160,82,0.2)] rounded-lg p-4">
        <p className="text-sm text-muted-wisdom">Write your manifestation as if it has already happened. Use present tense and vivid details.</p>
      </div>
      <div className="bg-elevated rounded-xl border-2 border-[rgba(196,160,82,0.2)] p-6">
        <textarea
          ...
          rows={15}
          className="w-full p-4 border rounded-lg focus:outline-none focus:ring-2 focus:ring-aged-gold"
          style={{ fontFamily: 'Georgia, serif', lineHeight: '1.8' }}
        />
        <p className="text-sm text-tertiary-text mt-2">{data.script.length} characters</p>
      </div>
    </div>
  )
}
