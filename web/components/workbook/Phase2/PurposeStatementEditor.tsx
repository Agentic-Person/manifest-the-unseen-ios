...
export function PurposeStatementEditor({ data, onChange, className = '' }: PurposeStatementEditorProps) {
  ...
  const getCharCountColor = () => {
    if (charCount === 0) return 'text-tertiary-text'
    if (charCount < recommendedMin) return 'text-muted-wisdom'
    if (charCount > maxChars * 0.9) return 'text-aged-gold'
    return 'text-aged-gold'
  }
    ...
    <div className={`space-y-8 ${className}`}>
      ...
      <div className="bg-elevated rounded-xl border border-[rgba(196,160,82,0.15)] p-6 shadow-[0_4px_24px_rgba(0,0,0,0.4)]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {GUIDING_QUESTIONS.map((item, index) => (
            <div key={index} className="bg-[rgba(26,26,36,0.8)] rounded-lg p-4 border border-[rgba(196,160,82,0.15)] shadow-[0_4px_24px_rgba(0,0,0,0.4)]">
              <div className="flex items-start gap-2">
                <span className="flex-shrink-0 w-6 h-6 bg-aged-gold text-temple-stone rounded-full flex items-center justify-center text-xs font-bold">
                  {index + 1}
                </span>
                <div className="flex-1">
                  <p className="text-xs font-semibold text-aged-gold mb-1">{item.category}</p>
                  <p className="text-sm text-muted-wisdom">{item.question}</p>
                </div>
              </div>
            </div>
          ))}
      ...
      <div className="bg-elevated rounded-xl border border-[rgba(196,160,82,0.15)] p-6 shadow-[0_4px_24px_rgba(0,0,0,0.4)]">
        ...
        <textarea
          ...
          className="w-full min-h-[250px] p-4 bg-[rgba(26,26,36,0.8)] border border-[rgba(196,160,82,0.15)] rounded-lg focus:outline-none focus:ring-2 focus:ring-aged-gold focus:border-transparent resize-y text-enlightened leading-relaxed"
          style={{ fontFamily: 'system-ui, sans-serif', fontSize: '16px' }}
        />

        {/* Progress Indicator */}
        <div className="mt-3">
          <div className="flex justify-between text-xs text-tertiary-text mb-1">
            <span>Progress</span>
            <span>
          ...
          <div className="w-full bg-[rgba(26,26,36,0.8)] rounded-full h-2">
            <div
              className={`h-2 rounded-full transition-all ${
                charCount >= recommendedMin ? 'bg-aged-gold' : 'bg-muted-wisdom'
              }`}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {/* Formula */}
        <div className="bg-elevated rounded-xl border border-[rgba(196,160,82,0.15)] p-6 shadow-[0_4px_24px_rgba(0,0,0,0.4)]">
          <h4 className="text-lg font-semibold text-enlightened mb-3 flex items-center gap-2">
            <svg className="w-5 h-5 text-aged-gold" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="space-y-3">
            <div className="bg-[rgba(26,26,36,0.8)] rounded-lg p-3 border border-[rgba(196,160,82,0.15)]">
              <p className="text-sm text-muted-wisdom">
                <span className="font-semibold text-aged-gold">Action Verb</span>
                <br />
                <span className="text-xs text-tertiary-text">inspire, empower, create, teach, heal, build...</span>
              </p>
            </div>
            <div className="text-center text-aged-gold font-bold">+</div>
            <div className="bg-[rgba(26,26,36,0.8)] rounded-lg p-3 border border-[rgba(196,160,82,0.15)]">
              <p className="text-sm text-muted-wisdom">
                <span className="font-semibold text-aged-gold">Who You Serve</span>
                <br />
                <span className="text-xs text-tertiary-text">others, my community, future generations...</span>
              </p>
            </div>
            <div className="text-center text-aged-gold font-bold">+</div>
            <div className="bg-[rgba(26,26,36,0.8)] rounded-lg p-3 border border-[rgba(196,160,82,0.15)]">
              <p className="text-sm text-muted-wisdom">
                <span className="font-semibold text-aged-gold">Desired Outcome</span>
                <br />
                <span className="text-xs text-tertiary-text">to live authentically, find peace, create change...</span>
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="bg-elevated border border-[rgba(196,160,82,0.15)] rounded-xl p-6 shadow-[0_4px_24px_rgba(0,0,0,0.4)]">
        ...
        <div className="space-y-3">
          <div className="bg-[rgba(26,26,36,0.8)] rounded-lg p-4 border border-[rgba(196,160,82,0.15)]">
            <p className="text-sm text-muted-wisdom">
              <span className="font-semibold text-aged-gold">Maya Angelou:</span>{' '}
          ...
          <div className="bg-[rgba(26,26,36,0.8)] rounded-lg p-4 border border-[rgba(196,160,82,0.15)]">
            <p className="text-sm text-muted-wisdom">
              <span className="font-semibold text-aged-gold">Example:</span>{' '}
